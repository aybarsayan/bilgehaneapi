...
app.post('/generate-questions-v2/:apiAnahtari', apiKeyMiddleware, async (req, res) => {
  const { category, subcategory, language_id, num_questions, topic, questionid, VECTOR_STORE_ID, language } = req.body;
  ...
  try {
      // Asistanı ve vektör deposunu alın veya oluşturun
      const assistant = await getOrCreateAssistant();
      const vectorStore = await getOrCreateVectorStore(VECTOR_STORE_ID);

      // Asistanı vektör deposuyla güncelleyin
      await updateAssistantWithVectorStore(assistant, vectorStore);

      // İleti dizisi oluşturun
      const thread = await createThread(userMessage);

      let fullResponse = '';

      // Asistan yanıtını streaming olarak alın
      const stream = getAssistantResponseStream(thread.id, assistant.id);

      stream.on('data', (data) => {
          // Gelen veriyi JSON olarak parse edin
          const parsedData = JSON.parse(data);

          // Yanıt içeriğini alın
          const content = parsedData.message?.content?.text?.value || '';

          if (content) {
              fullResponse += content;
              // Streaming sırasında gelen veriyi istemciye gönderin
              res.write(`data: ${JSON.stringify({ content })}\n\n`);
          }
      });

      stream.on('end', async () => {
          // Streaming tamamlandıktan sonra gelen tam yanıtı işleyin
          let trimmedAnalysis = fullResponse.replace(/\n/g, '');
          trimmedAnalysis = validateAndFixSQL(trimmedAnalysis);

          // SQL sorgusunu veritabanına yazdırın
          const pool = mysql.createPool({
              host: "ni-corona.guzelhosting.com",
              user: "bugraay1_bilgehaneadmin",
              password: "1071Aa1071bil15_",
              database: "bugraay1_bilgehane",
              waitForConnections: true,
              connectionLimit: 10,
              queueLimit: 0
          });
          ...
          pool.query(trimmedAnalysis, (error, results) => {
              if (error) {
                  console.error('SQL çalıştırma hatası:', error.stack);
                  res.write(`data: ${JSON.stringify({ error: 'SQL çalıştırılırken bir hata oluştu.', details: error.message })}\n\n`);
                  res.end();
                  // Bağlantıyı kapat
                  pool.end((err) => {
                      if (err) {
      ...
      stream.on('error', (error) => {
          console.error('Asistan yanıtı alınırken hata oluştu:', error);
          res.write(`data: ${JSON.stringify({ error: 'Bir hata oluştu', details: error.message })}\n\n`);
          res.end();
      });
